...
class AddParent extends Component
{
  ...
  protected $listeners = [
    ...
    public function submitForm()
    {
        ...
        $My_Parent->Name_Father = ['en' => $this->Name_Father_en, 'ar' => $this->Name_Father];// تخزين النصوص الإنجليزية والعربية معًا
        $My_Parent->National_ID_Father = $this->National_ID_Father;
        $My_Parent->Phone_Father = $this->Phone_Father;
        $My_Parent->Job_Father = ['en' => $this->Job_Father_en, 'ar' => $this->Job_Father];// تخزين النصوص الإنجليزية والعربية معًا
        ...
        // Mother_INPUTS
        $My_Parent->Name_Mother = ['en' => $this->Name_Mother_en, 'ar' => $this->Name_Mother];// تخزين النصوص الإنجليزية والعربية معًا
        $My_Parent->National_ID_Mother = $this->National_ID_Mother;
        $My_Parent->Passport_ID_Mother = $this->Passport_ID_Mother;
        $My_Parent->Phone_Mother = $this->Phone_Mother;
        $My_Parent->Job_Mother = ['en' => $this->Job_Mother_en, 'ar' => $this->Job_Mother]; // تخزين النصوص الإنجليزية والعربية معًا
        ...
        if (!empty($this->photos)) {
            foreach ($this->photos as $photo) {
                // تنظيف اسم المجلد
                $folderName = Str::slug($this->National_ID_Father, '_');
        
                // تخزين الملف
                $photo->storeAs($folderName, $photo->getClientOriginalName(), $disk = 'parent_attachments');
...
{
    // العثور على السجل الرئيسي
    $parent = My_Parent::findOrFail($id);

    // حذف المجلد المرتبط في Storage
    $folderName = Str::slug($parent->National_ID_Father, '_'); // اسم المجلد
    Storage::disk('parent_attachments')->deleteDirectory($folderName); // حذف المجلد بالكامل

    // حذف السجلات من جدول المرفقات
